...
interface Props {
  ...
  onSelectReport: (report: CommentReportRow) => void;
}
...
export function CommentReportTable({
  reports,
  isLoading,
  page,
  totalPages,
          ...
          >
            ...
            <p className="mt-2 line-clamp-2 text-xs text-muted-foreground">
              {report.comment_is_deleted ? (
                ...
                report.comment_body.slice(0, 120) +
                (report.comment_body.length > 120 ? "..." : "")
              )}
            </p>
            <div className="mt-1.5 flex items-center gap-3 text-xs text-muted-foreground">
              <span>{timeAgo(report.created_at)}</span>
              {report.warning_count > 0 && (
                <span className="text-yellow-600">{report.warning_count} warning{report.warning_count !== 1 ? "s" : ""}</span>
              )}
            </div>
          </button>
        ))}
              ...
              >
                ...
                <td className="max-w-[200px] truncate px-4 py-3 text-muted-foreground">
                  {report.comment_is_deleted ? (
                    ...
                    report.comment_body.slice(0, 80) +
                    (report.comment_body.length > 80 ? "..." : "")
                  )}
                </td>
