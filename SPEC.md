...
| 47d   | Events: recurring (weekly/monthly)                   | `DONE` | 2026-04-11. Migration `00049`: `event_series` table + `series_id`/`series_index` on `events` + RLS + `get_series_occurrences()` RPC. Materialized rows (up to 52 weekly / 12 monthly). `createEventSeries` action with occurrence generation. Scoped edit: "this only" / "this and following" (series split) / "all future". Scoped cancel: "this" / "all future" (cutoff = selected occurrence). Event form: recurring toggle (rrule, interval, until_date) on create; scope radio selector on edit. Series nav (prev/next) + "Recurring" badge on list and detail pages. RSVPs per-occurrence. |
...
| 48    | Visual: country flags + company logos on cards       | `DONE` | 2026-04-11. Emoji flags (250+ countries, case-insensitive with aliases). Logo.dev API proxy (`/api/logo/[domain]`, 7-day cache, SSRF-safe domain validation). `company_website` column on `career_entries` + career form field. `companyToDomain()` chain: website → 130-company static map → guess. `<CountryFlag>` server component, `<CompanyLogo>` client component with onError initial fallback. RPCs updated (migrations 00050–00051). Scope: directory, recommendation, popular, list item cards (flags+logos), connections, groups, profile (flags only). |
| 49    | UX: toast-before-navigation fix                      | `DONE` | 2026-04-11. Shared `useToastFromUrl()` hook reads toast from URL search params on destination page, fires Sonner toast, cleans params via `history.replaceState` (fires-once `useRef` guard). `<ToastFromUrl>` mounted in both `(main)` and `(auth)` layouts. `buildUrlWithToast()` helper in `src/lib/utils.ts`. 8 flows migrated across 6 files. Forms with navigating success state render a centered spinner placeholder (onboarding, signup, quick-update) to prevent form reset flash during transition. |
...
├── company_website (text, nullable — optional company URL for logo resolution)
