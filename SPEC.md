...
| 45a   | F45 gate test harness                                | `TODO` | First integration-test pattern for the codebase. Cover the 7 `can_see_last_seen()` scenarios (own profile, connected/no-msgs, connected/one-sided each way, connected/both-sent, hidden, not-connected) via pgTAP or Vitest hitting local Supabase. Will establish fixture seeding + isolation strategy for future SQL gate tests.                                                          |
...
| 47b   | Events: radius notifications                         | `TODO` | New `profiles.notify_events_within_km` preference (null = off). On public event create, notify verified users with geocoded coordinates inside radius. In-app via `notifyUser()` + email via Resend, hard-capped at 50/event and 80/day across all events; overflow degrades to in-app only. Email template. Tests for cap + radius filter. Depends on F47a.                                  |
| 47c   | Events: flat comment thread                          | `TODO` | `event_comments` table + RLS. Append-only thread on detail page visible to all event viewers. New comments notify host + prior commenters via `notifyUserGrouped()`. Report integration with existing moderation queue. Depends on F47a.                                                                                                                                                     |
...
| 47e   | Events: day-of QR check-in                           | `TODO` | `event_checkins` table. Host page renders rotating signed QR token (prevents reuse). `/events/[id]/checkin?token=…` scanner marks scanning attendee as `checked_in = true` if they hold a Going RSVP and current time is within the event window. Host sees attendance stats on detail page. Depends on F47a.                                                                                |
| 47f   | Events: group linkage + bulk-invite members          | `TODO` | New `events.group_id` FK (nullable). Group-linked events auto-visible to all members of the linked group (RLS). Bulk-invite action restricted to `group_members.role IN ('owner','moderator')`. Hard caps: max 100 members per bulk invite (groups >100 must use member picker) AND max 1 bulk invite per group per rolling 7 days. Invited members get in-app + email notification using the shared Resend daily budget. Group-linked invites do NOT get the +1 guest mechanic (+1 stays exclusive to creator-connection invites). Depends on F47a. |
